...
public class OneTrustPublishersNativeCmpPlugin implements FlutterPlugin, MethodCallHandler, ActivityAware {
  ...
  public void onMethodCall(@NonNull MethodCall call, @NonNull final Result result) {
    switch (call.method) {
      ...
      case "showUCPurposesUI":
        ot.showConsentPurposesUI((FragmentActivity) activity);
        break;
      case "getUCPurposeConsent":
        String purposeId = call.argument("forPurpose");
        ot.getUCPurposeConsent(purposeId);
        break;
      case "getUCPurposeTopicConsent":
        String purposeID = call.argument("forPurpose");
        String topicId = call.argument("forTopic");
        ot.getUCPurposeConsent(topicId, purposeID);
        break;
      case "getUCPurposeCustomPreferenceOptionConsent":
        String ucp = call.argument("forPurpose");
        String cpId = call.argument("forCustomPreference");
        String cpOptionId = call.argument("forCustomPreferenceOption");
        ot.getUCPurposeConsent(cpOptionId, cpId, ucp );
        break;
      case "updateUCPurposeConsent":
        String purpose = call.argument("forPurpose");
        boolean consentValP = call.argument("consentValue");
        ot.updateUCPurposeConsent(purpose, consentValP);
        break;
      case "updateUCPurposeTopicConsent":
        String purpose1 = call.argument("forPurpose");
        String topic1 = call.argument("forTopic");
        boolean consentValPT = call.argument("consentValue");
        ot.updateUCPurposeConsent(topic1, purpose1, consentValPT );
        break;
      case "updateUCPurposeCutomPreferenceOptionConsent":
        String purpose2 = call.argument("forPurpose");
        String cpOption1 = call.argument("forCpOption");
        String cp = call.argument("forCp");
        boolean consentValCpOption = call.argument("consentValue");
        ot.updateUCPurposeConsent(cpOption1, cp, purpose2, consentValCpOption );
        break;
      case "renameProfile":
        renameUserProfile(call, result);
        break;
      case "getConsentStatusForSDK":
        String sdkId = call.argument("forSdk");
        int sdkStatus = ot.getConsentStatusForSDKId(sdkId);
  ...
  private int getSDKLogLevel (String logLevel)  {
    int sdkLogLevel = Log.INFO ;
    switch (logLevel.toLowerCase()) {
      case "nologs":
      sdkLogLevel = OTLoggerConstant.NO_OT_SDK_LOG;
      break;
      case "debug":
      sdkLogLevel = Log.DEBUG;
      break;
      case "info":
      sdkLogLevel = Log.INFO;
      break;
      case "error":
      sdkLogLevel = Log.ERROR;
      break;
      case "warn":
      sdkLogLevel = Log.WARN;
      break;
      case "verbose":
      sdkLogLevel = Log.VERBOSE;
      break;
  ...
  private void renameUserProfile(@NonNull MethodCall call, @NonNull final Result result){
    String fromId = call.argument("fromIdentifier");
    /* We'll have a fromIdentifier no matter what, but it might be null. To take advantage of sound
    * null safety in Flutter, we'll replace the null value with an empty string. */
    if(fromId==null){
      fromId = "";
    }
    String toId = call.argument("toIdentifier");
    ot.renameProfile(fromId, toId, new OTCallback() {
      @Override
      ...
      public void onListen(Object arguments, final EventChannel.EventSink events) {
        ot.addEventListener(new OTEventListener() {
          ...
          public void onShowBanner(OTUIDisplayReason otuiDisplayReason) {
            events.success(new eventData("onShowBanner", otuiDisplayReason.logReason()).format());
          }
          ...
          public void onShowPreferenceCenter(OTUIDisplayReason otuiDisplayReason) {
            events.success(new eventData("onShowPreferenceCenter", otuiDisplayReason.logReason()).format());
          }
  ...
  private class eventData {
    ...
    eventData(String interactionType, String displayReason) {
      this.interactionType = interactionType;
